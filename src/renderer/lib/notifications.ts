...
export function notifyError(error: unknown, config?: { title?: string; fallback?: string; id?: string | number }) {
...
export function notifyValidation(message = "يرجى تصحيح الحقول المحددة ثم المحاولة مجددًا.") {
  return toast.warning("راجع البيانات المدخلة", { description: message, duration: 4500, closeButton: true });
}

export function notifySuccess(message: string, title = "تمت العملية بنجاح") {
  requestNotificationsRefresh();
  return toast.success(title, { description: message, duration: 3200, closeButton: true });
}

export function notifyWarning(message: string, title = "تنبيه") {
  return toast.warning(title, { description: message, duration: 4500, closeButton: true });
}

export function notifyInfo(message: string, title = "معلومة") {
  return toast.info(title, { description: message, duration: 3800, closeButton: true });
}

export function notifyLoading(message: string, id: string | number) {
  return toast.loading(message, { id, duration: Infinity, closeButton: false });
}
