...
export default function SalesTable({ sales, onView, onDelete }: { sales: SaleInvoiceRecord[]; onView: (sale: SaleInvoiceRecord) => void; onDelete: (sale: SaleInvoiceRecord) => void; }) {
  return <DataTable><DataTableHead><DataTableRow>{["رقم الفاتورة", "التاريخ", "العميل", "الإجمالي", "المدفوع", "المتبقي", "الحالة", "الإجراءات"].map((h) => <DataTableHeaderCell key={h}>{h}</DataTableHeaderCell>)}</DataTableRow></DataTableHead><DataTableBody>{sales.map((sale) => <DataTableRow key={sale.id}>
    <DataTableCell className="font-bold text-[var(--text-primary)]"><span className="inline-flex items-center gap-2">{sale.invoice_number}{Boolean(sale.is_edited) && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-800">معدلة</span>}</span></DataTableCell><DataTableCell>{sale.invoice_date}</DataTableCell><DataTableCell>{sale.customer_name ?? "بيع نقدي"}</DataTableCell><DataTableCell>{money(sale.total)}</DataTableCell><DataTableCell>{money(sale.paid_amount)}</DataTableCell><DataTableCell>{money(sale.remaining_amount)}</DataTableCell><DataTableCell><SaleStatusBadge status={sale.status as never} /></DataTableCell>
