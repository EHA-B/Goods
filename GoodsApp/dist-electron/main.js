...
const CONTROLLER_IPC_CHANNEL = "stocklite:controllers:invoke";
...
function getControllersDirectory() {
  if (app.isPackaged) {
    return path$1.join(process.resourcesPath, "src", "controllers");
  }
  return path$1.join(process.env.APP_ROOT ?? app.getAppPath(), "src", "controllers");
}
function getController(name) {
  const cached = controllerCache.get(name);
  if (cached) return cached;
  const definition = controllerDefinitions[name];
  const modulePath = path$1.join(getControllersDirectory(), definition.file);
  const loaded = require$1(modulePath);
...
function registerControllerIpc() {
  ...
  ipcMain.handle(
    ...
    async (_event, controllerName, methodName, args = []) => {
      ...
      if (!definition.methods.includes(methodName)) {
...
function createWindow() {
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    void win.loadFile(path$1.join(RENDERER_DIST, "index.html"));
  }
}
