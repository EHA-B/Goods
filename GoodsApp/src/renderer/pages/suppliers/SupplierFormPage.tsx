  ...
  return <>
    ...
    <Card header="بيانات المورد" description="البيانات الأساسية ومعلومات التواصل والحساب.">
      <div className="grid gap-5 md:grid-cols-2">
        <FormField label="اسم المورد" htmlFor="supplier-name" required error={nameError}><Input id="supplier-name" value={form.name} placeholder="اسم الشركة أو المورد" error={Boolean(nameError)} onChange={(e) => { update("name", e.target.value); if (nameError) setNameError(""); }} /></FormField>
        <FormField label="رقم الهاتف" htmlFor="supplier-phone"><Input id="supplier-phone" dir="ltr" value={form.phone} placeholder="09xxxxxxxx" onChange={(e) => update("phone", e.target.value)} /></FormField>
        <FormField label="البريد الإلكتروني" htmlFor="supplier-email"><Input id="supplier-email" dir="ltr" type="email" value={form.email} placeholder="supplier@example.com" onChange={(e) => update("email", e.target.value)} /></FormField>
        <FormField label="العنوان" htmlFor="supplier-address"><Input id="supplier-address" value={form.address} placeholder="المدينة، الحي، الشارع" onChange={(e) => update("address", e.target.value)} /></FormField>
        ...
        <div className="md:col-span-2"><FormField label="ملاحظات" htmlFor="supplier-notes"><Textarea id="supplier-notes" rows={5} value={form.notes} placeholder="ملاحظات وشروط التعامل مع المورد..." onChange={(e) => update("notes", e.target.value)} /></FormField></div>
      </div>
    </Card>
