...
const money = (value: number) => `${value.toLocaleString("en-US")} ل.س`;
export default function PrintableInvoice(props: Props) {
  return <article className="invoice-print-sheet" dir="rtl">
    ...
    <table className="invoice-items">
      ...
      <tbody>{props.items.map((item, index) => <tr key={item.id}><td className="invoice-index">{index + 1}</td><td className="invoice-product-name">{item.productName}</td><td><bdi>{item.batchCode || "—"}</bdi></td><td className="invoice-number-cell">{item.quantity.toLocaleString("en-US")}</td><td className="invoice-number-cell">{money(item.unitPrice)}</td><td className="invoice-number-cell invoice-line-total">{money(item.lineTotal)}</td></tr>)}</tbody>
    </table>
    ...
    <section className="invoice-bottom-grid">
      ...
      <div className="invoice-summary">
        <div><span>المجموع الفرعي</span><strong>{money(props.subtotal)}</strong></div>
        <div><span>الخصم</span><strong>{money(props.discount)}</strong></div>
        {props.extraAmount && <div><span>{props.extraAmount.label}</span><strong>{money(props.extraAmount.value)}</strong></div>}
        <div><span>الضريبة</span><strong>{money(props.tax)}</strong></div>
        <div className="invoice-total"><span>الإجمالي النهائي</span><strong>{money(props.total)}</strong></div>
        <div><span>المبلغ المدفوع</span><strong>{money(props.paidAmount)}</strong></div>
        <div className="invoice-remaining"><span>المبلغ المتبقي</span><strong>{money(Math.max(0, props.total - props.paidAmount))}</strong></div>
      </div>
    </section>
