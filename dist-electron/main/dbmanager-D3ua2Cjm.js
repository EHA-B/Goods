...
async function up$3(knex2) {
  const hasPurchaseItemsTable = await knex2.schema.hasTable(
    "purchase_invoice_items"
  );
  if (hasPurchaseItemsTable) {
    const hasEstimatedPurchasePrice = await knex2.schema.hasColumn(
      "purchase_invoice_items",
      "estimated_purchase_price"
    );
    if (!hasEstimatedPurchasePrice) {
      await knex2.schema.alterTable(
        "purchase_invoice_items",
        (table) => {
          table.decimal(
            "estimated_purchase_price",
  ...
  const hasStockBatchesTable = await knex2.schema.hasTable(
    "stock_batches"
  );
...
const consignmentExpectedPurchasePriceMigration = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  down: down$3,
  up: up$3
}, Symbol.toStringTag, { value: "Module" }));
...
async function up(knex2) {
  const hasWorkerPayments = await knex2.schema.hasTable("worker_payments");
  const hasTransactions = await knex2.schema.hasTable("transactions");
  if (!hasWorkerPayments || !hasTransactions) {
    return;
  }
  const hasTransactionId = await knex2.schema.hasColumn(
    "worker_payments",
    "transaction_id"
  );
  if (!hasTransactionId) {
    await knex2.schema.alterTable("worker_payments", (table) => {
...
const linkWorkerPaymentsFinancialTransactionsMigration = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  down,
  up
}, Symbol.toStringTag, { value: "Module" }));
...
class MigrationSource {
  ...
  async getMigration(migration) {
    ...
    if (migration === "20260807233500_consignment_expected_purchase_price.ts") {
