...
export default function ConsignmentOverviewPage() {
  ...
  return <>
    ...
    <Card className="mb-5"><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{([["المورد", summary.invoice.supplier_name], ["تاريخ الفاتورة", summary.invoice.invoice_date], ["العملة", summary.invoice.currency], ["المدفوع مسبقاً", summary.invoice.paid_amount > 0 ? `${summary.invoice.paid_amount.toLocaleString("en-US")} ${summary.invoice.currency}` : "—"], ["حالة التسوية", <ConsignmentStatusBadge status={summary.invoice.settlement_status} />], ...(summary.invoice.transport_cost > 0 ? [[`تكلفة النقل (${summary.invoice.transport_cost_bearer === "supplier" ? "على المورد" : "علينا"})`, `${summary.invoice.transport_cost_bearer === "supplier" ? "− " : "+ "}${summary.invoice.transport_cost.toLocaleString("en-US")} ${summary.invoice.currency}`]] : []), ...(summary.invoice.emptying_cost > 0 ? [[`تكلفة العتالة (${summary.invoice.emptying_cost_bearer === "supplier" ? "على المورد" : "علينا"})`, `${summary.invoice.emptying_cost_bearer === "supplier" ? "− " : "+ "}${summary.invoice.emptying_cost.toLocaleString("en-US")} ${summary.invoice.currency}`]] : [])] as [string, React.ReactNode][]).map(([label, value]) => <div key={String(label)}><p className="text-xs font-bold text-[var(--text-muted)]">{label}</p><div className="mt-2 text-sm font-black">{value}</div></div>)}</div></Card>
