...
export default function CloseConsignmentPage() {
  ...
  useEffect(() => { void (async () => { try { setError(""); const [s, c] = await Promise.all([consignmentService.getSummary(id), consignmentService.getCashboxes()]); setSummary(s); setCashboxes(c.map((c: any) => ({ ...c, isActive: !!c.isActive }))); const matching = c.find((x) => x.currency === s.invoice.currency); if (matching) setCashboxId(String(matching.id)); } catch (e) { setError(getArabicErrorMessage(e, "تعذر تحميل بيانات التسوية.")); } finally { setLoading(false); } })(); }, [id]);
  const input = useMemo<CloseConsignmentInput>(() => ({ commission_percentage: Number(commission), cashbox_id: Number(cashboxId), settlement_date: date, remaining_stock_policy: policy, exchange_rate: Number(exchangeRate) || undefined, notes: notes.trim() || null }), [commission, cashboxId, date, policy, exchangeRate, notes]);
  useEffect(() => { if (!summary || !cashboxId) return; void consignmentService.getClosingPreview(id, input).then((value) => { setPreview(value); setError(""); }).catch((e) => { setPreview(null); setError(getArabicErrorMessage(e, "تعذر حساب معاينة التسوية.")); }); }, [summary, id, input, cashboxId]);
  ...
  const selectedCashbox = cashboxes.find((c) => String(c.id) === cashboxId);
  const requiresExchangeRate = selectedCashbox && selectedCashbox.currency !== summary.invoice.currency;
  const submit = async () => { try { setSubmitting(true); setError(""); if (!preview?.calculation_hash) throw new Error("يجب تحديث المعاينة قبل التسوية."); const result = await consignmentService.close(id, { ...input, calculation_hash: preview.calculation_hash }); navigate(`/purchases/${id}/consignment-settlement`, { state: { settlement: result } }); } catch (e) { setError(getArabicErrorMessage(e, "تعذر إتمام التسوية.")); } finally { setSubmitting(false); setConfirmOpen(false); } };
  return <>
      ...
      <div className="space-y-5">
        ...
        <Card header="بيانات التسوية">
          <div className="grid gap-4 sm:grid-cols-2">
            <FormField label="نسبة العمولة" required><Input type="number" min="0" max="100" value={commission} onChange={(e) => setCommission(e.target.value)} endContent={<span className="px-3 text-sm text-[var(--text-muted)]">%</span>} /></FormField>
            <FormField label="تاريخ التسوية" required><Input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></FormField>
            <FormField label="الصندوق" required><Select value={cashboxId} onChange={(e) => setCashboxId(e.target.value)} placeholder="اختر الصندوق" options={cashboxes.map((cashbox) => ({ value: String(cashbox.id), label: `${cashbox.name} — ${money(cashbox.balance, cashbox.currency)}` }))} /></FormField>
            {requiresExchangeRate && (
              <FormField label="سعر الصرف" required>
                <Input type="number" min="0" step="any" value={exchangeRate} onChange={(e) => setExchangeRate(e.target.value)} placeholder={`سعر صرف ${selectedCashbox.currency} مقابل ${summary.invoice.currency}`} />
              </FormField>
            )}
            <FormField label="الملاحظات" className={requiresExchangeRate ? "sm:col-span-2" : ""}><Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="ملاحظات اختيارية حول التسوية" /></FormField>
          </div>
        </Card>
