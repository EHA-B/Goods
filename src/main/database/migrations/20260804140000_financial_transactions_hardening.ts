...
export async function up(knex: Knex): Promise<void> {
  ...
  const afterCount = await knex('transactions_new').count('* as cnt').first();
...
export async function down(knex: Knex): Promise<void> {
  await knex.schema.createTable('transactions_orig', (table) => {
    table.increments('id').primary();
    table.integer('category_id').unsigned().notNullable();
    table.integer('cashbox_id').unsigned().notNullable();
    table.decimal('amount', 15, 2).notNullable();
  ...
  await knex.schema.dropTable('transactions');
  await knex.schema.renameTable('transactions_orig', 'transactions');
