...
export default function SecuritySettingsPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (code === "INVALID_CURRENT_PASSWORD") {
        setErrors((current) => ({ ...current, current: "كلمة المرور الحالية غير صحيحة." }));
  ...
  const passwordEnd = (field: PasswordField) => (
    <button
      ...
      aria-label={visible[field] ? "إخفاء كلمة المرور" : "إظهار كلمة المرور"}
    >
  ...
  return (
    <form onSubmit={submit} className="space-y-6" noValidate>
      <PageHeader
        ...
        actions={<BackButton to={PATHS.SETTINGS} label="العودة إلى الإعدادات" />}
      />

      <Card header="بيانات الحساب" description="الحساب الحالي محفوظ في قاعدة البيانات ولا تُعرض كلمة المرور أو تُخزّن داخل الواجهة.">
        <div className="flex items-center gap-3 rounded-[var(--radius-md)] bg-[var(--surface-subtle)] p-4">
          ...
          <div>
            <p className="font-bold text-[var(--text-primary)]">{user?.full_name ?? "مستخدم النظام"}</p>
            <p dir="ltr" className="mt-0.5 text-left text-xs text-[var(--text-muted)]">{user?.username}</p>
          </div>
        </div>
      </Card>

      <Card header="تغيير كلمة المرور" description="استخدم كلمة مرور قوية لا تقل عن 8 محارف.">
        <div className="grid gap-5 md:grid-cols-2">
          <div className="md:col-span-2">
            <FormField label="كلمة المرور الحالية" required error={errors.current}>
              <Input
          ...
          <FormField label="كلمة المرور الجديدة" required error={errors.next}>
            <Input
          ...
          <FormField label="تأكيد كلمة المرور الجديدة" required error={errors.confirm}>
            <Input
      ...
      <div className="flex justify-end">
        <Button type="submit" isLoading={isSaving} loadingText="جاري الحفظ..." startIcon={<Save size={17} />}>
          حفظ كلمة المرور
        </Button>
      </div>
    </form>
